...
def calculate_strategy_grade_static(strategy_name):
    ...
    # If strategy not found, check if it's one of our predefined ones with different casing
    for strategy, (grade, multiplier) in strategy_grades.items():
        if strategy_name.upper() in strategy.upper() or strategy.upper() in strategy_name.upper():
            return grade, multiplier
